...
public class Client {
    ...
    public static void main(String[] args) {
        ...
        if (args != null) {
            for (String a : args) {
                ...
                if (a.startsWith("--client-cache-cap=")) {
                    try {
                        CLIENT_CACHE_CAP = Integer.parseInt(a.substring(a.indexOf('=') + 1).trim());
        ...
        if (!CLIENT_CACHE_ENABLED)
            CLIENT_CACHE_ENABLED = parseBoolEnv("CLIENT_CACHE", false);
        int capEnv = parseIntEnv("CLIENT_CACHE_CAP", CLIENT_CACHE_CAP);
        if (capEnv > 0)
            CLIENT_CACHE_CAP = capEnv;
        try {
            ...
            try (FileWriter mw = new FileWriter(metricsPath, false)) {
                mw.write(
                        "method,args,client_zone,server_zone,value,status,wait_ms,exec_ms,turn_ms,total_ms,start_ms,end_ms\n");
            }
        } catch (IOException e) {
    ...
    private static String selectOutputPath() {
        ...
        String base;
        if (CLIENT_CACHE_ENABLED) {
            base = "client_cache.txt";
        } else if (parseBoolEnv("SERVER_CACHE", false)) {
            base = "server_cache.txt";
        } else {
            base = "naive_server.txt";
        }
        if (configured != null && !configured.isBlank()) {
            File f = new File(configured);
            File dir = f.getParentFile();
            return dir != null ? new File(dir, base).getPath() : base;
        }
        return base;
    }
    ...
    private static String deriveMetricsPath(String outputPath) {
        try {
            ...
            String base = out.getName(); // e.g., client_cache.txt | server_cache.txt | naive_server.txt
            String metricsName = "metrics_" + base + ".csv"; // postfix CSV with the .txt filename
            return (dir != null) ? new File(dir, metricsName).getPath() : metricsName;
        } catch (Exception ignore) {
            return "metrics.csv";
        }
    }
    ...
    private void parseInputFile() {
        try {
                ...
                switch (methodName) {
                    ...
                    case "getNumberofCities", "getNumberofCountries" -> {
                        ...
                        if (parts.length >= 3) {
                            ...
                            if (c.equals(">") || c.equals("=") || c.equals("min"))
                                compInt = 1;
                            else if (c.equals("<") || c.equals("max"))
                                compInt = 2;
                            else
                                continue;
                        }
            ...
            logger.log(Level.SEVERE,
                    "Input file not found: " + System.getenv().getOrDefault("INPUT_PATH", "exercise_1_input.txt"), e);
        }
    }
}
